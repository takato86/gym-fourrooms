...
from gym_fourrooms.envs.shaping_reward import DefaultReward
...
class Rooms(gym.Env):
    ...
    def __init__(self):
        ...
        # From any state the agent can perform one of four actions, up, down,
        # left or right
...
"""
    ...
    def step(self, action):
        ...
        With probability 2/3, the actions cause the agent to move one cell
        in the corresponding direction, and with probability 1/3,
        ...
        if not self.occupancy[nextcell]:
            if self.rng.uniform() < 1/3.:
                ...
                self.currentcell = empty_cells[
                    self.rng.randint(len(empty_cells))
                ]
            else:
    ...
    def draw_square(self, start_x, start_y, length_x, length_y, color):
        ...
        cell = rendering.make_polygon(
            [(start_x, start_y), (start_x+length_x, start_y),
             (start_x + length_x, start_y + length_y),
             (start_x, start_y+length_y)])
        if color_dict.get(color) is not None:
            cell.set_color(
            ...
            for j in range(self.occupancy.shape[1]):
                if self.occupancy[i][j] == 1:
                    ...
                    if self.curr_option is not None:
                        ...
                        color = 'blue'
                    self.currentcell_obj = self.draw_square(
...
class FlexibleRooms(Rooms):
    ...
    def set_init_states(self, init_states):
        ...
        """
        self.init_states = [
            self.tostate[init_state] for init_state in init_states
        ]
